package com.sjl.bookmark.widget.reader.loader;
...
public abstract class PageLoader {
    ...
    // 书本对象
    protected CollectBook mCollBook;
    // 监听器
    ...
    /*****************************init params*******************************/
    public PageLoader(PageView pageView, CollectBook collBook) {
        mPageView = pageView;
    ...
    private void initData() {
        ...
        // 初始化参数
        mMarginWidth = ViewUtils.dp2px(MyApplication.getContext(), DEFAULT_MARGIN_WIDTH);
        mMarginHeight = ViewUtils.dp2px(MyApplication.getContext(), DEFAULT_MARGIN_HEIGHT);
        // 配置文字有关的参数
    ...
    private void setUpTextParams(int textSize) {
        // 文字大小
        mTextSize = textSize;
        mTitleSize = mTextSize + ViewUtils.sp2px(MyApplication.getContext(), EXTRA_TITLE_SIZE);
        // 行间距(大小为字体的一半)
    ...
    private void initPaint() {
        ...
        mTipPaint.setTextSize(ViewUtils.sp2px(MyApplication.getContext(), DEFAULT_TIP_SIZE)); // Tip默认的字体大小
     ...
     * 设置夜间模式
     *
     * @param nightMode true夜间模式，false日间模式
     */
     ...
     * @return
     */
    public CollectBook getCollBook() {
        return mCollBook;
    }
    ...
    private void prepareBook() {
        mBookRecord = DaoFactory.getBookRecordDao()
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        int tipMarginHeight = ViewUtils.dp2px(MyApplication.getContext(), 3);
        if (!isUpdate) {
            /****绘制背景****/
            ...
            //擦除区域
            mBgPaint.setColor(mBgColor);
            canvas.drawRect(mDisplayWidth / 2, mDisplayHeight - mMarginHeight + ViewUtils.dp2px(MyApplication.getContext(), 2), mDisplayWidth, mDisplayHeight, mBgPaint);
        }
        ...
        int polarHeight = ViewUtils.dp2px(MyApplication.getContext(), 6);
        int polarWidth = ViewUtils.dp2px(MyApplication.getContext(), 2);
        int border = 1;
        int innerMargin = 1;
        ...
        Rect polar = new Rect(polarLeft, polarTop, visibleRight,
                polarTop + polarHeight - ViewUtils.dp2px(MyApplication.getContext(), 2));
        ...
        int outFrameBottom = visibleBottom - ViewUtils.dp2px(MyApplication.getContext(), 2);
        ...
        //底部的字显示的位置Y
        float y = mDisplayHeight - mTipPaint.getFontMetrics().bottom - tipMarginHeight;
        String time = TimeUtils.formatDateToStr(System.currentTimeMillis(), TimeUtils.DATE_FORMAT_3);
        float x = outFrameLeft - mTipPaint.measureText(time) - ViewUtils.dp2px(MyApplication.getContext(), 4);
    ...
    private void preLoadNextChapter() {
        ...
        }).compose(RxSchedulers.<List<TxtPage>>applySingle())
    ...
    private List<TxtPage> loadPages(TxtChapter chapter, BufferedReader br) {
        ...
        try {
            while (showTitle || (paragraph = br.readLine()) != null) {
                paragraph = WordUtils.convertCC(paragraph, mContext);
                // 重置段落
                if (!showTitle) {
                    paragraph = paragraph.replaceAll("\\s", "");
                    // 如果只有换行符，那么就不执行
                    if (paragraph.equals("")) continue;
                    paragraph = WordUtils.halfToFull("  " + paragraph + "\n");
                } else {
                    //设置 title 的顶部间距
                    rHeight -= mTitlePara;
                }
                ...
                while (paragraph.length() > 0) {
                    ...
                    if (rHeight <= 0) {
                        ...
                        page.title = WordUtils.convertCC(chapter.title, mContext);
    ...
    private void refreshMemoryData() {
        ...
        if (mChapterList != null) {
            for (TxtChapter txtChapter : mChapterList) {
                txtChapter.title = WordUtils.convertCC(txtChapter.title, mContext);
            }
        }
        // 当前显示的页
        if (mCurPage != null) {
            mCurPage.title = WordUtils.convertCC(mCurPage.title, mContext);
            List<String> lines = new ArrayList<>(mCurPage.lines.size());
            for (String text : mCurPage.lines) {
                lines.add(WordUtils.convertCC(text, mContext));
            }
            mCurPage.lines = lines;
        }
        // 上一章的页面列表缓存
        if (mPrePageList != null) {
            convertCC(mPrePageList);

        }
        // 当前章节的页面列表
        if (mCurPageList != null) {
            convertCC(mCurPageList);
        }
        // 下一章的页面列表缓存
        if (mNextPageList != null) {
            convertCC(mNextPageList);
        }
    }

    private void convertCC(List<TxtPage> txtPages) {
        for (TxtPage txtPage : txtPages) {
            txtPage.title = WordUtils.convertCC(txtPage.title, mContext);
            List<String> lines = new ArrayList<>(txtPage.lines.size());
            for (String text : txtPage.lines) {
                lines.add(WordUtils.convertCC(text, mContext));
            }
            txtPage.lines = lines;
        }
